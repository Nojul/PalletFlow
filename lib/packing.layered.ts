...
const MIN_FLOOR_SUPPORT_FRACTION = 0.6;
const MAX_CANDIDATES_PER_PLACEMENT = 800;
...
function getRequiredSupportFraction(
  candidate: PlacedBox,
  minSupportFraction: number,
) {
  if (candidate.z <= GEOMETRY_EPS) {
    return Math.max(minSupportFraction, MIN_FLOOR_SUPPORT_FRACTION);
  }

  return minSupportFraction;
}
...
function scoreCompactGridPlan(
  ...
  const filledArea = placements.reduce(
    (sum, box) => sum + box.width * box.depth,
    0,
  );
  const minX = Math.min(...placements.map((box) => box.x));
  const maxX = Math.max(...placements.map((box) => box.x + box.width));
  const minY = Math.min(...placements.map((box) => box.y));
  const maxY = Math.max(...placements.map((box) => box.y + box.depth));
  const boundingArea = Math.max(GEOMETRY_EPS, (maxX - minX) * (maxY - minY));
  const unusedBoundingArea = Math.max(0, boundingArea - filledArea);

  const minAllowedX = surface.x - overhangX;
  const minAllowedY = surface.y - overhangY;
  const maxAllowedX = surface.x + surface.width + overhangX;
  const maxAllowedY = surface.y + surface.depth + overhangY;

  const leftGap = Math.max(0, minX - minAllowedX);
  const frontGap = Math.max(0, minY - minAllowedY);
  const rightGap = Math.max(0, maxAllowedX - maxX);
  const backGap = Math.max(0, maxAllowedY - maxY);

  return (
    placements.length * 2_000_000 +
    filledArea * 500 -
    unusedBoundingArea * 900 -
    (leftGap + frontGap) * 600 -
    (rightGap + backGap) * 120
  );
}

function buildCompactGridPlanForVariant(
  surface: TopSurface,
  placed: PlacedBox[],
  entry: FoundationPlanEntry,
  ...
  const startXCandidates = xAxis.slice(0, Math.min(6, xAxis.length));
  const startYCandidates = yAxis.slice(0, Math.min(6, yAxis.length));

  let bestPlan: PlacedBox[] = [];
  let bestScore = -Infinity;

  for (const startX of startXCandidates) {
    for (const startY of startYCandidates) {
      const localPlaced: PlacedBox[] = [];

      for (const y of yAxis) {
        if (y < startY - GEOMETRY_EPS) continue;
        for (const x of xAxis) {
          if (x < startX - GEOMETRY_EPS) continue;
          if (localPlaced.length >= entry.remaining) break;
          ...
          if (localPlaced.some((other) => collide(candidate, other))) continue;

          if (!hasSurfaceAdjacency(candidate, localPlaced)) continue;
          ...
          if (
            candidate.z > GEOMETRY_EPS &&
            !hasSupportAnchorAlignment(candidate, [...placed, ...localPlaced])
          ) {
            continue;
          }
      ...
      const score = scoreCompactGridPlan(
        localPlaced,
        surface,
        overhangX,
        overhangY,
      ...
      if (score > bestScore) {
        bestPlan = localPlaced;
        bestScore = score;
      }
    }
  }

  if (bestPlan.length === 0) return null;

  return {
    placements: bestPlan,
    usedByType: { [entry.template.id]: bestPlan.length },
    count: bestPlan.length,
    filledArea: bestPlan.reduce((sum, box) => sum + box.width * box.depth, 0),
  };
}
...
function solveSurfaceMiniPacking(
  ...
  // Prefer flat orientations and a single orientation/type footprint grid.
  for (let uprightPass = 0; uprightPass < 2; uprightPass++) {
    ...
    for (const entry of inventory) {
      ...
      for (const variant of variants) {
        ...
        const plan = buildCompactGridPlanForVariant(
          surface,
          placed,
          entry,
          variant,
        ...
        if (
          !bestPlan ||
          plan.count > bestPlan.count ||
          (plan.count === bestPlan.count &&
            plan.filledArea > bestPlan.filledArea)
        ) {
          bestPlan = plan;
        }
      }
  ...
  return (
    bestPlan ?? {
      placements: [] as PlacedBox[],
      usedByType: {} as Record<string, number>,
      count: 0,
      filledArea: 0,
    }
  );
...
function fillTopSurfacesMiniLayer(
  ...
  while (keepPacking) {
    ...
    for (const surface of topSurfaces) {
      ...
      const bestPlan = solveSurfaceMiniPacking(
        surface,
        placed,
        inventory,
        pallet,
      ...
      keepPacking = true;
      placed.push(...bestPlan.placements);
      ...
      for (const candidate of bestPlan.placements) {
...
function stabilizePlaced(
  ...
  for (const box of sorted) {
    ...
    if (currentSupport >= getRequiredSupportFraction(box, minSupportFraction)) {
      box.invalid = false;
    ...
    for (const z of lowerZ) {
      ...
      if (
        support >= getRequiredSupportFraction(box, minSupportFraction) &&
        z >= 0
      ) {
        snapped = true;
        box.invalid = false;
...
// Layered algorithm:
// 1) Compute foundation plan: how many of each type must be on the first layer.
// 2) Build first layer with ONLY those required base boxes (no overfilling).
// 3) For each type, stack remaining copies vertically on top of their bases.
// 4) Fill remaining gaps with leftover boxes.
export function buildLayeredPackingPlan(
  pallet: PalletConfig,
  boxes: BoxTemplate[],
  options: PackingOptions = {},
) {
  ...
  const inventory = createFoundationPlan(boxes, pallet).sort(
    (a, b) =>
      b.footprintArea - a.footprintArea ||
      b.maxStackHeight - a.maxStackHeight ||
  ...
  // Phase 1: global footprint-first layer planning.
  const firstLayer = buildStackFootprintLayer(pallet, inventory);
  if (firstLayer.length > 0) {
    placed.push(...firstLayer);
  }

  // Track per-type inventory from actual first-layer results.
  for (const entry of inventory) {
    const floorCount = placed.filter(
      (box) => box.boxId === entry.template.id && Math.abs(box.z) < 1e-6,
    ).length;
    entry.remaining = Math.max(0, entry.quantity - floorCount);
  }

  // Phase 2: aggressively fill floor while useful space remains.
  // This prevents early height growth when the first layer still has valid placements.
  while (true) {
    let best: {
      entry: FoundationPlanEntry;
    ...
    for (const entry of inventory) {
      ...
      const placement = findBestStrictScannableFloorPlacement(
        item,
        placed,
        pallet,
        minSupportFraction,
      ...
      if (
        !best ||
        placement.score > best.placement.score ||
        (placement.score === best.placement.score &&
          placement.coords.x < best.placement.coords.x)
      ) {
    ...
    placed.push(best.placement.candidate);
    best.entry.remaining -= 1;
  }

  // Phase 3: build each primary stack up to max stack height.
  // Use round-robin to consider every valid base surface before exhausting one column.
  for (const entry of inventory) {
    const bases = placed.filter(
      (box) => box.boxId === entry.template.id && Math.abs(box.z) < 1e-6,
    );

    const stackState = bases.map((base) => ({
      current: base,
      count: 1,
    }));

    const item = createFootprintDimensionItem(entry);

    while (entry.remaining > 0) {
      let placedInPass = false;

      for (const stack of stackState) {
        if (entry.remaining <= 0) break;
        if (stack.count >= entry.maxStackHeight) continue;
        ...
        entry.remaining -= 1;
        stack.count += 1;
        stack.current = stackPlacement.candidate;
        placedInPass = true;
      }
  ...
  // Phase 4a: prioritize top surfaces of first-layer boxes.
  fillTopSurfacesMiniLayer(
    placed,
    inventory,
    pallet,
    minSupportFraction,
  ...
  // Phase 4b: then use any remaining elevated surfaces.
  fillTopSurfacesMiniLayer(placed, inventory, pallet, minSupportFraction);

  const sanitized = sanitizeLayeredPlacements(
    placed,
    pallet,
    minSupportFraction,
  );
  const collisionFree = enforceNoOverlap(sanitized);

  return annotateVisibility(pallet, assignStackLevels(collisionFree));
}
